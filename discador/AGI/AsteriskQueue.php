    ...
    if($Multi==0 && $Pause>0){
        ...
        if($Validar > 0){
            while($row = mysql_fetch_array($Comenzar)){
                ...
                $Id = $row[0];
                $Rut = $row[2];
                $ContarOnline = mysql_num_rows(mysql_query("SELECT * FROM Asterisk_InCall WHERE Queue = $NombreQueue"));
                if($ContarOnline>=$Multi){
                    ...
                    mysql_query("INSERT INTO Asterisk_InCall(Fono,Rut,Queue) VALUES ('$Fono','$Rut','$NombreQueue')");
                    mysql_query("UPDATE $ColaAsterisk SET llamado = 1 WHERE Fono = $Fono");
                    echo $FonoSip = "SIP/".$Fono."@claro";
                    $asm = new AGI_AsteriskManager();
                    $asm->connect("127.0.0.1","lponce","lponce");
                    echo $VarAgi = $Id."&".$Fono."&".$NombreQueue."&".$Rut."&".$Cedente;
                    $resultado = $asm->originate("$FonoSip","$NombreQueue","from-prueba","1","","","18000","227144101","$VarAgi","$VarAgi","false","1001");
                    sleep(1);
                    print_r($resultado);
                    $Stop = Stop();
                    $asm->disconnect();
