                ...
                <div class="cols-sm-6 col-sm-offset-2">
                    ...
                    <div class="col-sm-3">
                        <div class="form-group">
                            ...
                            <select class="selectpicker form-control" name="Test" title="Seleccione" multiple="" data-live-search="true" data-width="100%"></select>
                        </div>
                    </div>
                </div>
            </div>
        </script>
        <script id="UpdatePruebaTemplate" type="text/template">
            <div class="row">
                <div class="col-sm-6">
                    <div class="form-group">
                        <label class="control-label">Perfiles:</label>
...
<script>
    $(document).ready(function(){
        ...
        $("body").on("click",".Delete",function(){
            ...
            bootbox.confirm("¿Desea eliminar la prueba?",function(result){
                if(result){
                    DeletePrueba(id);
                }
            })
        ...
        $("body").on("click",".Update",function(){
            ...
            var ObjectDiv = ObjectMe.closest("div");
            var id = ObjectDiv.attr("id");
            var Template = $("#UpdatePruebaTemplate").html()
            bootbox.dialog({
            ...
            fillPerfiles();
            fillTests();
            var Prueba = getPruebaData(id);
            $("select[name='Perfil']").val(Prueba.id_perfil);
            $("select[name='Perfil']").selectpicker("refresh");
            $("select[name='Test']").val(Prueba.id_tipotest);
        ...
        function DeletePrueba(idPrueba){
            $.ajax({
                ...
                success: function(data){
                    ...
                    switch(json.result){
                        case '0':
                            bootbox.alert("Hubo un error al eliminar la prueba. Intentelo nuevamente o consulte con el administrador.");
                        break;
                        case '1':
                            location.reload();
                        break;
        ...
        function updatePrueba(idPrueba){
            var idPerfil = $("select[name='Perfil']").val();
            var idTest = $("select[name='Test']").val();

            $.ajax({
                type: "POST",
                url: "ajax/updatePrueba.php",
                dataType: "html",
                data: {
                    idPrueba: idPrueba,
                    idPerfil: idPerfil,
                ...
                success: function(data){
                    ...
                    if(json.result){
                        ...
                        bootbox.alert("Hubo un error al modificar la prueba. Intentelo nuevamente o consulte con el administrador.");
                    }
                },
                error: function(){
                }
