...
function transcurrido(){
	...
	else
	{
		...
		function funcionMostrarGestionCorreo(data_gestion_total)
		{
			$.ajax(
			{
				...
				success: function(response)
				{
					$('#mostrar_gestion_correo').html(response);
											...
											$('#seleccione_nivel3').change(function()
											{
												...
												if (nombreNivel1 != 'INBOUND'){
													...
													if($('#ultimo_fono').val()==0){
														...
														mensa = "Debe finalizar la Gestión para poder guardar una nueva gestión!";
													}
												}else{
													...
													if ($('#rut_buscado').val() == ''){
														bandera = 0;
														mensa = "Debe buscar un Rut para poder guardar una gestión!";
												...
												else
												{
													...
													var rut_ultimo = $('#rut_ultimo').val();
													var nivel4 = "id_tipo="+$('#tipo_gestion_final').val()+"&cortar_valor="+cortar_valor+"&rut="+rut_ultimo;
													$.ajax(
													{
														type: "POST",
														...
														success: function(response)
														{
															...
															{																
																...
																if (GlobalData.tipoFactura == 1){																																		
																	if ($('#tipo_gestion_final').val() == 5){
																		facturas = $('#facturas').val();
																		if ((facturas == 0) || (facturas == "") || (facturas == null))
																		{
																			alert('Debe seleccionar minimo una Factura');
																			return 0;
																		}
																	}  
																}														
																...
																if(tipo_gestion2==136 || tipo_gestion2==37)
																{
																	...
																	$.ajax(
																	{
																		...
																		{												
																			...
																			if ((rutEstrategia == 2) && (GlobalData.tipoFactura == 0)){
							...
							else
							{
								...
								$.ajax(
								{
									...
									success: function(response)
									{
										...
										$('#tablaTelefonos tr:last').after('<tr id="'+fila+'"><td class="text-sm"><center><i class="fa fa-flag fa-lg icon-lg" style="color:#ff0080"></i> </center></td><td class="text-sm">Nuevo Fono</td><td class="text-sm"><input type="hidden" id="telefono'+fila+'" value="'+fono_discado_nuevo+'" name="telefono'+fila+'">'+fono_discado_nuevo+'</td><td class="text-sm">'+fechaCarga+'</td><td class="text-sm"><center><input type="checkbox" disabled  class="fono_gestion" name="llamado'+fila+'" value="llamado'+fila+'" id="llamado'+fila+'" ></center></td><td class="text-sm"><center><button id="fono'+fila+'" class="btn btn-success btn-icon icon-lg fa fa-phone Llamar"  value="Llamar"></button></center></td></tr>');
									}
								});
		...
		$(document).on('change', '#seleccione_asignacion', function(){
			...
			$.ajax({
				...
				success: function(response){
					console.log(response);	
					$("#reinicioAsignacion").val(0);
					$("#reinicioAsignacion").selectpicker("refresh");
		...
		$(document).on('change', '#reinicioAsignacion', function(){
			var Cola_Final = $('#seleccione_asignacion').val();
			var tipo = $('#reinicioAsignacion').val();
			$('#nuevo_telefono').prop("disabled",false);
			$('#nuevo_direccion').prop("disabled",false);
			$('#nuevo_correo').prop("disabled",false);
			$.ajax({
				type: "POST",
				url: "../includes/crm/reiniciarAsignacion.php", // "rut="+rut+"&prefijo="+prefijo;
				data:"Prefijo="+Cola_Final+"&tipo="+tipo, 
				dataType: 'json',
				success: function(response){
					console.log(response);	
					$('#ocultar_rut').hide();
		...
		$(document).on('click', '#next_rutSiguiente', function(){		
			...
			var data = "rut="+rut+"&prefijo="+prefijo+"&orden="+ordenRut;
			console.log(data);
			$.ajax({
				type: "POST",
			...
			recognition.onresult = function(event){
				for (var i = event.resultIndex; i < event.results.length; i++) {
					if(event.results[i].isFinal)
						valor = event.results[i][0].transcript;
						console.log(valor);
						$('#Hablar').val(valor);
				}
			}
	...
	$("#EnviarFacturaCorreos").click(function(){
		...
		if(Cont > 0){
			...
			bootbox.alert("Debe seleccionar almenos un correo electronico");
		}
	});
