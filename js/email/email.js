$(document).ready(function(){
    ...
    $('#enviar-mail').on('click', function(){
        ...
        if(cant > 0){
			$.ajax({
				...
				data: { est:est, cant:cant, asunto:asunto, html:html, adjuntar, tablaEnvio:tablaEnvio},
				dataType: "html",
				beforeSend: function(){
				...
				success: function(result){
					if(result==1){
						...
						$(".alert").html("Error al intentar envío de correos.");
					}
				},
				error: function(){
					alert('error');
    ...
    $('#save-temp').on('click', function(){	
		...
		$("#message").css("display","block");
		if ((template == '<p><br></p>') || (template == "") || (tname == "")){
			 $(".alert").addClass("alert-danger");
			 $(".alert").html("Debe ingresar todos los datos");
			 setTimeout(function() { 
			$("#message").fadeOut(1000);
			$(".alert").html(""); 
			$(".alert").removeClass("alert-danger");
		...
		$.ajax({
			...
			success: function(result){	
				if(result == 1){	
					...
					bootbox.alert("Configuración guardada satisfactoriamente");
				}
			},
			error: function(){
				$(".alert").removeClass("alert-success");
	...
	$("body").on("change",".inputCheckFactura",function(){
		...
		ChangeFacturaTemplate(ID);
	});
	function ChangeFacturaTemplate(idTemplate){
		$.ajax({
			type: "POST",
