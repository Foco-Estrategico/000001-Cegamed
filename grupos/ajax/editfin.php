 	...
 	if ($resultado) {
		...
		mysql_query('DELETE FROM grupos_personas WHERE IdGrupo = '.$_POST['id']);
 		for ($i=0; $i < count($personas); $i++) {
 			echo 'INSERT INTO grupos_personas (IdGrupo, Rut) VALUES ("'.$_POST['id'].'", "'.$personas[$i].'")';
			$resultado = mysql_query('INSERT INTO grupos_personas (IdGrupo, Rut) VALUES ("'.$_POST['id'].'", "'.$personas[$i].'")');
 		}
 		for ($i=0; $i < count($empresas); $i++) {
 			echo 'INSERT INTO grupos_empresas (IdGrupo, IdEmpresaExterna) VALUES ("'.$_POST['id'].'", "'.$empresas[$i].'")';
