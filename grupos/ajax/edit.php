<?php
	...
	$num2 = count($rows);
	for ($i=0; $i < count($rows) ; $i++) {
		$lista2.= "<div class='row t1'><div class='col-md-3 form-group'><span att='".$rows[$i]['IdEmpresaExterna']."' class='form-control bg-info empresasEx'>".$rows[$i]['Nombre']."</span></div><div class='col-md-1'><button type='button' class='btn btn-danger dlt'><i class='ti-close'></i></button></div></div>";
	}
	$listas[0] = $lista;
	$listas[1] = $lista2;
	$listas[2] = $data;
