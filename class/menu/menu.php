...
class Menu
{
  ...
  public function __construct($idMenu, $nivelUsuario)
  {
    ...
    $this->tipoSistema = $this->getTipoSistema();
                    ...
                    echo "<li class='".$classActiveSub."'>";
                      ...
                      // <!--Submenu -->
                        ...
                        while ($resultNivel2 = mysql_fetch_array($resultadoNivel2))
                        {
                          ...
                          if (($this->isAuthorized($resultNivel2["permisos"])) && ($this->BlockMenu_ByName($resultNivel2["nombre"])))
                          {
                            $classActiveLink = "";
                            $classActiveSub = "";
  ...
  public function consultaMenu($idPadre)
  {
    ...
    $conn = mysql_connect($host_name, $user_name, $pass_word) or die ('Error connecting to mysql');
    mysql_select_db($database_name);
    $consultaMenu = "SELECT * FROM menu WHERE padre = ".$idPadre." and find_in_set(".$this->tipoSistema.",tipoSistema) order by prioridad";
    //$consultaMenu = "SELECT * FROM menu WHERE padre = ".$idPadre." order by prioridad";
  ...
  {
    $db = new DB();
    $consultaMenu = "SELECT tipoSistema from focoConfig";
    $resultado = $db->select($consultaMenu);
    $resultado = $resultado[0];
    return $resultado["tipoSistema"];
  }
}
?>
