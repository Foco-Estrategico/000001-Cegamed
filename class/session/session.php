<?php
  class Session
  {
    ...
    public function login($loginUsername,$password,$conn,$database_name) // Listo
    {
      ...
      $MM_redirectLoginSuccess3 = "cedente.php";
      ...
      $LoginRS__query=sprintf("SELECT usuario, clave, nivel, nombre, id, sexo,email,cargo,anexo_foco,INBOUND FROM Usuarios WHERE usuario=%s",
      ...
      if ($loginFoundUser) // si entro aca es porque el usuario existe
      {
        ...
        if($objetoHash->verificarHash($password,$row['clave']))
        {
          ...
          $this->sexo = $row['sexo'];
          $this->anexo_foco = $row['anexo_foco'];
          $this->inbound = $row['INBOUND'];
          ...
          // OJO esta creando las variables de session asi el nivel del usuario no exista
            ...
            $array = array(
              ...
              "tipoSistema" => $this->getTipoSistema()
            );
    ...
    function getTipoSistema(){
      ...
      $TipoSistema = mysql_query($SqlTipoSistema);
      while($Row = mysql_fetch_assoc($TipoSistema)){
        $ToReturn = $Row["tipoSistema"];
      }
      return $ToReturn;
    }
  }
