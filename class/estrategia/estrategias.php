...
{    
    ...
    public function MostrarColor(){
        ...
        $QueryColor= mysql_query("SELECT id,nombre,comentario FROM SIS_Colores ");
    ...
    public function CrearQuery($Valor,$Logica,$NombreCola,$IdColumna,$IdCedente,$IdEstrategia,$IdSubQuery,$IdTabla,$Color)
    {
        $this->Valor=$Valor;
        $findme   = ',';
        ...
        $QueryDocumentosPositivos = mysql_query("SELECT Rut FROM Deuda WHERE Rut IN ($QueryPositiva) and Id_Cedente = $this->IdCedente");
        $CantidadDocumentosPositivos = mysql_num_rows($QueryDocumentosPositivos);

        $QueryExecNegativa = mysql_query($QueryNegativa);
        $CantidadRegistrosNegativos = mysql_num_rows($QueryExecNegativa);

        $QueryDocumentosNegativos = mysql_query("SELECT Rut FROM Deuda WHERE Rut IN ($QueryNegativa) and Id_Cedente = $this->IdCedente");
        $CantidadDocumentosNegativos = mysql_num_rows($QueryDocumentosNegativos);

        $QueryDeudaPositiva = "SELECT SUM( d.Saldos) FROM Persona p, Deuda d WHERE p.Rut IN ($QueryPositiva) AND p.Rut = d.Rut AND d.Id_Cedente = $this->IdCedente";
        ...
        $QueryDeudaNegativa = "SELECT SUM( d.Saldos) FROM Persona p, Deuda d WHERE p.Rut IN ($QueryNegativa) AND p.Rut = d.Rut AND d.Id_Cedente =  $this->IdCedente";
        ...
        $InsertarQueryPostiva = mysql_query("INSERT INTO SIS_Querys_Estrategias (query,monto,cola,cantidad,id_estrategia,query_resumen,condicion,id_subquery,Id_Cedente,columna,espacios,dinamica,color,documentos) VALUES ('$QueryPositiva','$MontoMoraPositiva','$NombreCola','$CantidadRegistrosPositivos','$this->IdEstrategia','$QueryResumen','Rut IN','$this->IdSubQuery','$this->IdCedente','$Columna','$NumeroEspaciosTotal','$Dinamica','$Color','$CantidadDocumentosPositivos')");
        $InsertarQueryNegativa = mysql_query("INSERT INTO SIS_Querys_Estrategias (query,monto,cola,cantidad,id_estrategia,query_resumen,condicion,id_subquery,Id_Cedente,columna,espacios,dinamica,color,documentos) VALUES ('$QueryNegativa','$MontoMoraNegativa','Resto','$CantidadRegistrosNegativos','$this->IdEstrategia','$QueryResumen','NOT Rut IN','$this->IdSubQuery','$this->IdCedente','$Columna','$NumeroEspaciosTotal','$Dinamica','$Color','$CantidadDocumentosNegativos')");
    ...
    public function MostrarEstrategias($IdEstrategia)
    {
        ...
        else
        {
            ...
            echo '<tr>';
            echo "<th>Grupo</th>";
            echo '<th class="min-desktop"><center>Cant. Rut</center></th>';
             echo '<th class="min-desktop"><center>Cant. Documentos</center></th>';
            ...
            while($j<$contar)
            {
                ...
                $sq = mysql_query("SELECT cola,id,espacios,cantidad,monto,dinamica,carpeta,prioridad,comentario,terminal,sistema,documentos FROM SIS_Querys_Estrategias WHERE id=$arre");
                while($row=mysql_fetch_array($sq))
                {
                    ...
                    {
                        if($Sistema==1){
                            echo "<td class='bg-gray-dark'>".$EspaciosHtmlTotal.$Carpeta."<input type='text' id='K$Id' class='text-transparent-left Cola ' value='$Cola'></td>";
                            echo "<td class='bg-gray-dark'><center><input type='text' readonly class='text-transparent-min' value='$Cantidad'></center></td>";
                            echo "<td class='bg-gray-dark'><center><input type='text' readonly class='text-transparent-min' value='$Documentos'></center></td>";

                            echo "<td class='bg-gray-dark'><center><input type='text' readonly class='text-transparent-max' value='$ $Monto'></center></td>";
                            echo "<td class='bg-gray-dark'><center><span ><input type='text'  class='text-transparent-min Prioridad' id='P$Id' value='$Prioridad'></center></td>";
                            echo "<td class='bg-gray-dark'><center><span ><input type='text'  class='text-transparent-max Comentario' id='C$Id' value='$Comentario'></center></td>";
                            echo "<td class='bg-gray-dark'><center>$Dividir</center></td>";
                            echo "<td class='bg-gray-dark'><center><input type='checkbox'  $Check class='Terminal' id='T$Id'></center></td>";
                        }
                        else{
                            ...
                            echo "<td><center><input type='text' readonly class='text-transparent-min' value='$Documentos'></center></td>";
                    ...
                    else
                    {
                        ...
                        echo "<td class='bg-muted'><span class='$Sucess'>".$EspaciosHtmlTotal.$Carpeta."<input type='text' id='K$Id' class='text-transparent-left Cola' value='$Cola'></span></td>";
                        echo "<td class='bg-muted'><center><span class='$Sucess'><input type='text' class='text-transparent-min' value='$Cantidad'></span></center></td>";
                        echo "<td class='bg-muted'><center><span class='$Sucess'><input type='text' class='text-transparent-min' value='$Documentos'></span></center></td>";

                        echo "<td class='bg-muted'><center><span class='$Sucess'><input type='text' class='text-transparent-max' value='$ $Monto'></span></center></td>";
                        echo "<td class='bg-muted'><center><span class='$Sucess'><input type='text'  class='text-transparent-min Prioridad' id='P$Id' value='$Prioridad'></span></center></td>";
                        echo "<td class='bg-muted'><center><span class='$Sucess'><input type='text'  class='text-transparent-max Comentario' id='C$Id' value='$Comentario'></span></center></td>";
                        echo "<td class='bg-muted'><center><span class='$Sucess'>$Dividir</span></center></td>";
                        echo "<td class='bg-muted'><center><span class='$Sucess'><input type='checkbox'  $Check class='Terminal' id='T$Id'></span></center></td>";
    ...
    public function Deshacer($IdEstrategia)
    {
        ...
        while($row = mysql_fetch_array($QueryDeshacer))
        {
            ...
            mysql_query("DELETE FROM SIS_Querys_Estrategias WHERE id_subquery = $IdSubqueryDesahacer AND id_estrategia = $this->IdEstrategia and sistema =0");
    ...
    public function crearEstrategia($nombre_estrategia,$tipo_estrategia,$comentario,$fecha,$hora,$usuario,$cedente,$idUsuario)
	{
        ...
        $IdEstrategia = '';
        echo "INSERT INTO SIS_Estrategias(nombre,comentario,fecha,hora,usuario,tipo,Id_Cedente,Id_Usuario) VALUES('$this->nombre_estrategia','$this->comentario','$this->fecha','$this->hora','$this->usuario','$this->tipo_estrategia',$this->cedente,$this->idUsuario)";
		...
		$query1=mysql_query("SELECT id FROM SIS_Estrategias WHERE nombre='$this->nombre_estrategia' and fecha='$this->fecha' AND hora = '$this->hora' LIMIT 1");
        ...
        $QueryPositiva = "SELECT Rut FROM Persona WHERE Rut IN (SELECT Rut FROM fono_cob WHERE CHAR_LENGTH(formato_subtel) =9) AND FIND_IN_SET($this->cedente,Id_Cedente)";
        $QueryNegativa = "SELECT Rut FROM Persona WHERE NOT Rut IN (SELECT Rut FROM fono_cob WHERE CHAR_LENGTH(formato_subtel) =9) AND FIND_IN_SET($this->cedente,Id_Cedente)";

        $QueryExecPositiva = mysql_query($QueryPositiva);
        $CantidadRegistrosPositivos = mysql_num_rows($QueryExecPositiva);

        $QueryDocumentosPositivos = mysql_query("SELECT Rut FROM Deuda WHERE Rut IN ($QueryPositiva) and Id_Cedente = $this->cedente");
        $CantidadDocumentosPositivos = mysql_num_rows($QueryDocumentosPositivos);

        $QueryExecNegativa = mysql_query($QueryNegativa);
        $CantidadRegistrosNegativos = mysql_num_rows($QueryExecNegativa);

        $QueryDocumentosNegativos = mysql_query("SELECT Rut FROM Deuda WHERE Rut IN ($QueryNegativa) and Id_Cedente = $this->cedente");
        $CantidadDocumentosNegativos = mysql_num_rows($QueryDocumentosNegativos);

        $QueryDeudaPositiva = "SELECT SUM( d.Saldos) FROM Persona p, Deuda d WHERE p.Rut IN ($QueryPositiva) AND p.Rut = d.Rut AND d.Id_Cedente = $this->cedente";
        $QueryDeudaExecPositiva = mysql_query($QueryDeudaPositiva);
        $MontoMoraPositiva = '';
        while($row = mysql_fetch_array($QueryDeudaExecPositiva))
        {
        ...
        $QueryDeudaNegativa = "SELECT SUM( d.Saldos) FROM Persona p, Deuda d WHERE p.Rut IN ($QueryNegativa) AND p.Rut = d.Rut AND d.Id_Cedente =  $this->cedente";
        $QueryDeudaExecNegativa = mysql_query($QueryDeudaNegativa);
        $MontoMoraNegativa = '';
        while($row = mysql_fetch_array($QueryDeudaExecNegativa))
        {
      ...
      //  mysql_query("UPDATE SIS_Querys_Estrategias SET carpeta='1' WHERE id=$this->IdSubQuery");
        ...
        $InsertarQueryPostiva = mysql_query("INSERT INTO SIS_Querys_Estrategias (query,monto,cola,cantidad,id_estrategia,query_resumen,condicion,id_subquery,Id_Cedente,columna,espacios,dinamica,sistema,documentos) VALUES ('$QueryPositiva','$MontoMoraPositiva','Con Telefonos','$CantidadRegistrosPositivos','$IdEstrategia','$QueryResumen','Rut IN','0','$this->cedente','','','','1','$CantidadDocumentosPositivos')");
        $InsertarQueryNegativa = mysql_query("INSERT INTO SIS_Querys_Estrategias (query,monto,cola,cantidad,id_estrategia,query_resumen,condicion,id_subquery,Id_Cedente,columna,espacios,dinamica,sistema,documentos) VALUES ('$QueryNegativa','$MontoMoraNegativa','Sin Telefonos','$CantidadRegistrosNegativos','$IdEstrategia','$QueryResumen','NOT Rut IN','0','$this->cedente','','','','1','$CantidadDocumentosNegativos')");
	}
    ...
    public function RecalculaQuery(){
        ...
        while($row = mysql_fetch_array($SelectQuery)){
            ...
            $ExecQuery = mysql_query("SELECT SUM( d.Saldos) FROM Persona p, Deuda d WHERE p.Rut IN ($Query) AND p.Rut = d.Rut");
...
WHERE FIND_IN_SET( r.Rut, t.Rut ) GROUP BY t.id ");
    ...
    public function RecalculaQueryCedente($Cedente,$IdEstrategia){
        ...
        $SelectQuery = mysql_query("SELECT id,query,Id_Cedente FROM SIS_Querys_Estrategias WHERE Id_Cedente = $this->Id_Cedente AND id_estrategia=$IdEstrategia");
        $Fecha = date('Y-m-d');
        while($row = mysql_fetch_array($SelectQuery)){
            ...
            */
            $CantidadRegistros = mysql_num_rows(mysql_query($Query));
            mysql_query("UPDATE SIS_Querys_Estrategias SET cantidad=$CantidadRegistros WHERE id=$Id");
            /*$QueryConcat = mysql_query("SELECT  GROUP_CONCAT(Rut) FROM Persona WHERE Rut IN($Query) AND FIND_IN_SET($Id_Cedente,Id_Cedente)");
            while($row = mysql_fetch_array($QueryConcat)){
                $Concat = $row[0];
            }
