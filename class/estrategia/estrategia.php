...
class Estrategia
{
	...
	public function mostrarRelacion()
	{
		...
		$query_id1=mysql_query("SELECT id FROM SIS_Querys_Estrategias WHERE query='$query1' AND id_estrategia=$this->id_estrategia");
		$query_id2=mysql_query("SELECT id FROM SIS_Querys_Estrategias WHERE query='$query2' AND id_estrategia=$this->id_estrategia");
	...
	function getColas($Estrategia){
		$ToReturn = "";
		$Query = mysql_query("SELECT id,cola FROM SIS_Querys_Estrategias WHERE id_estrategia = '".$Estrategia."'");
		while($row = mysql_fetch_assoc($Query))
		{
            $id = $row["id"];
            $cola = $row["cola"];
	...
	public function mostrarTabla($lista,$periodo)
	{
		...
		if($this->lista == -1)
		{
			...
			echo '<th class="min-tablet">Ultima Gestion</th>';
		    ...
		    $q6 = mysql_query("SELECT Rut FROM Ultima_Gestion WHERE Id_Cedente = $this->cedente ");
		    ...
		    while($r = mysql_fetch_array($q2))
		    {
		        ...
		        echo "</td>";
		        echo "<td>";
		        $q4 = mysql_query("SELECT Rut FROM Ultima_Gestion WHERE Respuesta_N1 = $rid AND Id_Cedente = $this->cedente ");
		...
		else
		{
		    ...
		    $total =  mysql_num_rows($q1);
		    $q6 = mysql_query("SELECT Rut FROM Ultima_Gestion WHERE Id_Cedente = $this->cedente and lista=$this->lista");
		    $cg =  mysql_num_rows($q6);
		    $i = 0;
		    $q2 = mysql_query("SELECT Id,Respuesta_N1 FROM Nivel1 WHERE FIND_IN_SET('$this->cedente',Id_Cedente)");
		    while($r = mysql_fetch_array($q2))
		    {
		        ...
		        echo "</td>";
		        echo "<td>";
		        $q4 = mysql_query("SELECT Rut FROM Ultima_Gestion WHERE Respuesta_N1 = $rid AND Id_Cedente = $this->cedente and lista=$this->lista");
