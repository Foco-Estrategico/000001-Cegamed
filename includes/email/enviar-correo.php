...
<body>
'.$html.'
<img src="http://foco.cobranding.cl/email/confirmar-lectura.php?code='.$code.'&e=[correo]" style="opacity: 0; height:1px; width:1px;">
</body>
</html>';
...
$consulta_pendientes = "SELECT * FROM envio_email WHERE status = 0 and Id_Cedente='".$cedente."'";
...
if(count($pendientes) > 0){
	...
	if($existe){
		...
		echo '1'; // Error, Ya existe un envío programado para la estrategia
		...
		$programar_envio = "INSERT INTO envio_email(estrategia, cantidad, offset, asunto, html, actualizacion, adjuntar, Id_Cedente, tabla_email, id_usuario, fechaProceso) VALUES ('".$estrategia."','".$cantidad."','0','".$asunto."','".$html."','".$date."','".$adjuntar."','".$cedente."','".$tablaEmail."','".$idUsuario."','NOW()')";
	...
	$consultar_tiempo = "SELECT actualizacion FROM envio_email WHERE status = 1 and Id_Cedente='".$cedente."' ORDER BY id DESC LIMIT 1";
	...
	if(count($tiempo) > 0){
		...
		if(strtotime($hora_envio) <= strtotime($hora_actual)){
			...
			if ($tablaEmail == 1){
				$select_correos = "SELECT m.correo_electronico, m.Rut FROM Mail_cedente m , ".$estrategia." q WHERE m.Rut = q.Rut AND Id_Cedente = ".$cedente." LIMIT 500";			
			}else{
				if ($tablaEmail == 0){
					$select_correos = "SELECT m.correo_electronico, m.Rut FROM Mail m , ".$estrategia." q WHERE m.Rut = q.Rut LIMIT 500";
				}
			}
			...
			if(count($correos) > 0){
				...
				$envio_result = $envio->SendMail($html,$asunto,$correos_array, $info,$cedente);
				...
				if($envio_result){
					...
					$agregar_pendientes = "INSERT INTO envio_email(estrategia, cantidad, asunto, html, offset, status, actualizacion, adjuntar, Id_Cedente, tabla_email, id_usuario, fechaProceso) VALUES ('".$estrategia."','".$cantidad."','".$asunto."','".$html."','".$offset."','".$status."','".$date."','".$adjuntar."','".$cedente."','".$tablaEmail."','".$idUsuario."','NOW()')";
					...
					if($agregar !== false){
						...
						$select_last_id = "SELECT id FROM envio_email where Id_Cedente='".$cedente."' ORDER BY id DESC";
						$last_id = $db->select($select_last_id);
		...
		if(count($correos) > 0){
			...
			if($envio_result){
				...
				$update_pendientes = "INSERT INTO envio_email(estrategia, cantidad, asunto, html, offset, status, actualizacion, adjuntar, Id_Cedente, tabla_email, id_usuario, fechaProceso) VALUES ('".$estrategia."','".$cantidad."','".$asunto."','".$html."','".$offset."','".$status."','".$date."','".$adjuntar."','".$cedente."','".$tablaEmail."','".$idUsuario."','NOW()')";
